...
const getUserBookings = async (req, res) => {
    ...
    try {
        ...
        const bookings = await Booking.find({ userId }) // Ensure you're using the correct field for user ID
             // Adjust if necessary to match your schema
...
const getAllBookings = async (req, res) => {
    try {
        const bookings = await Booking.find() // Fetch all bookings and populate hotel details
        res.status(200).json(bookings);
    } catch (error) {
...
module.exports = { createHotelBooking,getAllBookings,getHotelBookingById,getUserBookings  };
